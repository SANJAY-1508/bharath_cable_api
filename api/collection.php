...
if (isset($obj['search_text'])) {
    ...
    $sql = "SELECT 
            ...
            WHERE c.`deleted_at` = 0 AND cu.`name` LIKE ?";
    ...
    if (!empty($collection_paid_date) && !empty($area_id) && !empty($customer_id) && !empty($plan_id) && !empty($box_no) && !empty($current_user_id) && !empty($entry_amount) && !empty($plan_prize)) {
        ...
        if (!empty($current_user_name)) {
            $total_collection_months = $entry_amount / $plan_prize; // Still calculate for customer table

            // **Validations**
            $month = date('Y-m', strtotime($collection_paid_date));  // Month: YYYY-MM
            $day = date('Y-m-d', strtotime($collection_paid_date));  // Day: YYYY-MM-DD
